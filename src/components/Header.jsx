    ...
    lg:bg-n-8/90 lg:backdrop-blur-sm ${openNavigation ? 'bg-n-8' : 'bg-n-8/90 backdrop-blur-sm'}`}>
        <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
            ...
            lg:static lg:flex lg:mx-auto lg:bg-transparent`}>
                <div className="relative z-2 flex flex-con items-center justify-center m-auto
                lg:flex-row">
                    {navigation.map((item) => (
                        <a key={item.id} href={item.url} className={`block relative font-code
                        ...
                        ${item.url === pathname.hash ? 'z-2 lg:text-n-1' : 'lg:text-n-1/50'} lg:hover:text-n-1
                        xl:px-12`} onClick={handleClick}>
                            {item.title}
                        </a>
                    ))}
                </div>
            ...
            <a href="#singup" className='button hidden mr-8 text-n-1/50 transition-colors hover:text-n-1
            lg:block'>
                New account
            </a>
            ...
            <Button className="ml-auto lg:hidden" px="px-3" onClick={toggleNavition}>
